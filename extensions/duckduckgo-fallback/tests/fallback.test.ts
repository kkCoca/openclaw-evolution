...
import { DEFAULT_FALLBACK_CONFIG, GeminiSearchProvider, SmartSearchError, AuditLogFields, AuditEventType } from '../src/types.js';
...
function createLogger(events: LoggedEvent[]) {
  return {
    info(event: AuditEventType, fields: AuditLogFields) {
      events.push({ event, fields });
    },
    warn(event: AuditEventType, fields: AuditLogFields) {
      events.push({ event, fields });
    },
    error(event: AuditEventType, fields: AuditLogFields) {
      events.push({ event, fields });
    },
  };
}
