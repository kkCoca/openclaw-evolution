...
sync_all_mirrors() {
    ...
    for ((i=0; i<mirrors_count; i++)); do
        ...
        if ! is_mirror_enabled $i; then
            log_debug "仓库 $(get_mirror_name $i) 未启用，跳过"
            continue
        fi
        
        # 检查分支是否需要同步
        if ! should_sync_branch $i; then
            log_debug "分支 $current_branch 不在同步列表，跳过 $(get_mirror_name $i)"
            continue
        fi
        ...
        if [ -z "$url" ]; then
            log_error "仓库 $i 的 URL 为空，跳过"
            FAIL_COUNT=$((FAIL_COUNT + 1))
            continue
        fi
        
        # 执行同步（带重试）
        if sync_with_retry "$url" "$name" "$current_branch"; then
