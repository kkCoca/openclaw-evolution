...
export class FallbackManager {
    ...
    async executeSearch(input) {
        ...
        try {
            ...
            if (!this.config.enabled || classification.decision === 'fatal') {
                ...
                this.logger.error('search.failed_final', this.buildLog(context, elapsedMs, classification, 'gemini', 'search.failed_final', error.message));
    ...
    async executeDdgFallback(input, context, classification, geminiError) {
        ...
        this.logger.error('search.failed_final', this.buildLog(context, this.now() - context.startedAt, classification, 'ddg', 'search.failed_final', lastDdgError?.message ?? finalError.message, context.attempts.ddg));
        throw finalError;
    }
    /**
     * 创建致命错误 (不触发 Fallback)
     */
    createFatalError(classification, eventId) {
     ...
     * 构建审计日志字段 (EventID 挂载)
     */
    buildLog(context, elapsedMs, classification, finalProvider, eventType = 'search.request.started', errorMessage, retryCount = Math.max(0, context.attempts.ddg - 1)) {
        return {
            eventId: context.eventId,
