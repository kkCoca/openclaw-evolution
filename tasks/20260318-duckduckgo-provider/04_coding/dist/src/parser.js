...
export class DuckDuckGoHtmlParser {
    parse(html) {
        ...
        if (results.length === 0) {
            throw new ProviderError('PARSE_ERROR', 'PARSE_ERROR: no valid DuckDuckGo results could be extracted. Check if the response contains a CAPTCHA challenge.', false);
        }
        return results;
    }
    /**
