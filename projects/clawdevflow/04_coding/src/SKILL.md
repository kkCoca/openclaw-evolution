...
**流程引擎包含多次审阅等待环节，sessions_spawn timeout 必须足够长**

| 配置项 | 推荐值 | 说明 |
|--------|--------|------|
| `timeoutSeconds` | **86400**（24小时） | 与内部审阅等待 timeout 保持一致 |
| 内部审阅 timeout | 86400000ms（24小时） | review-workflow.js 内置 |
...
sessions_spawn({
  runtime: "subagent",
  task: "任务描述...",
  timeoutSeconds: 86400  // 24小时，确保审阅等待不被中断
})
```

**原因说明**：
- 流程引擎有 8 个阶段，每个阶段可能需要审阅等待
- 审阅等待时间取决于用户响应速度（可能数小时到数天）
- sessions_spawn timeout 过短会导致流程引擎在审阅等待期间被终止
- 审阅结果未处理 → approved 快照未生成 → resume 时门禁校验失败
