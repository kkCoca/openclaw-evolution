...
export class AttachmentParser {
     ...
     * 提取附件列表
     */
    async extractAttachments(page) {
        const attachments = [];
        try {
            // 查找附件区域（多种可能的选择器）
            const attachmentAreas = await page.$$('.attachment-area, .file-list, .attachments, [class*="attachment"]');
            for (const area of attachmentAreas) {
                // 查找附件项
                const items = await area.$$('.attachment-item, .file-item, [class*="file-item"]');
     ...
     * 解析单个附件项
     */
    async parseAttachmentItem(item) {
        try {
            // 提取文件名
            const name = await item.$eval('.file-name, .file-title, [class*="file-name"]', (el) => el.textContent?.trim() || '').catch(() => '');
            // 提取文件大小
            const size = await item.$eval('.file-size, .size, [class*="file-size"]', (el) => el.textContent?.trim() || '').catch(() => '');
            // 提取下载链接（致远 OA 中通常为空）
     ...
     */
    generateBatchDownloadUrl(fileIds, zipFileName) {
        if (fileIds.length === 0) {
            return '';
        }
     ...
     * 格式化附件列表为 Markdown
     */
    formatAttachmentsAsMarkdown(attachments) {
        ...
        for (const attachment of attachments) {
            const downloadText = attachment.needsManualDownload
                ? '需手动下载'
                : `[下载](${attachment.url})`;
            lines.push(`| ${attachment.name} | ${attachment.size} | ${downloadText} |`);
        }
        // 添加批量下载提示
